...
import { getClients, getHome } from "@/lib/content";
...
export async function ClientMarquee() {
  const [clients, home] = await Promise.all([getClients(), getHome()]);
  return (
    <section
    ...
    >
      <Container className="flex flex-col gap-5 py-7 lg:flex-row lg:items-center lg:gap-10">
        <span className="label-mono shrink-0 text-paper/55">{home.clientsLabel}</span>
